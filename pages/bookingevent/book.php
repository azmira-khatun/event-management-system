...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['customer_name'])) {
  ...
  if ($event) {
    ...
    $date = $event['date'];
    // $rent = $event['rent']; // This line is not needed if you don't calculate final rent

    // Final rent calculation is removed
    // $finalRent = $rent - $discountRent;

    // Insert into booking table
    // REMOVED 'final_rent' FROM THE COLUMN LIST
    $stmt = $conn->prepare("
            INSERT INTO booking (event_id, venue_id, date, customer_name, gmail, contact_number, address, image, discount_rent)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
    ...
    if (!$stmt) {
      ...
      // REMOVED 'd' FROM THE TYPE STRING AND $finalRent VARIABLE
      $stmt->bind_param(
        "iissssssi",
        $event_id,
        $venue_id,
